...
def _load_data(basedir, factor=None, width=None, height=None, load_imgs=True):
    ...
    poses[:2, 4, :] = np.array(sh[:2]).reshape([2, 1])  # adjust H and W
    poses[2, 4, :] = poses[2, 4, :] * 1./factor  # adjust focal length
...
def get_flow_grid(H, W, num_img, flows_f, flow_masks_f, flows_b, flow_masks_b):
...
def load_llff_data(args, basedir,
    ...
    # scale the scene (why use this scale factor?)
    poses[:, :3, 3] *= sc
    bds *= sc
    ...
    grids = get_flow_grid(int(H), int(W), len(poses), flows_f, flow_masks_f, flows_b, flow_masks_b) # [N, H, W, 8]
